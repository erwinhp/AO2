...
$.ajax({
    ...
    success:function(data)
    {
      ...
      for(var count=0; count < data.length; count++)
      {
        ...
        html +='<tr>';
        html +='<td>'+(count+1)+'</td>';
        html +='<td  class="column_name" data-column_name="uraian" data-id="'+data[count].id_cda+'">'+data[count].laporan+'</td>';
        html +='</tr>';
        $('#tablerab').html(html);
      }
    }
...
function postchart()
{
  $.ajax({
      type: 'POST',
      url: '/storelaporankons',
      Async:"False",
      data: {
        "_token": "<?php echo e(csrf_token()); ?>",
        // '_token': $('input[name=_token]').val(),
        'no_rab': $('#rab_select').val(),
        'laporan': $('#summernote').val(),
      },
      success:function(data)
      {
          alert("sukses input");
...
<?php echo $__env->make('layouts.indexNVM', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\git\AO2\resources\views/Konstruksi/laporan_pengawas.blade.php ENDPATH**/ ?>