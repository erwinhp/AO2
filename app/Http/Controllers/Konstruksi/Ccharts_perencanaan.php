...
class Ccharts_perencanaan extends Controller
{
  public function index()
  {
    ...
    {
      $norab=DB::select('select tgl_progress, jumlah_progress FROM chart_perencanaan where no_rab=:a',['a'=>$request->getrab]);
      $sums=[];
      $sum=0;
      ...
      {
        $norab=DB::select('select sum(jumlah_progress)as getsum FROM chart_perencanaan where no_rab=:a',['a'=>$request->getrab]);
